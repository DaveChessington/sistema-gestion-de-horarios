...
class Evento(db.Model):
    ...
    # Estandarizado a 'fecha' para coincidir con Peticion
    ...
    id_peticion = db.Column(db.Integer, db.ForeignKey('reservas.peticion.id_peticion'), nullable=True)
    
    prioridad = db.Column(db.Integer, nullable=False, default=0)
    activo = db.Column(db.Boolean, default=True, nullable=False)

    # Relaciones
    tipo_evento = db.relationship('TipoEvento', backref='eventos', lazy=True)

    @property
    def fecha_apartado(self):
        """Alias de compatibilidad por si se usa en vistas previas"""
        return self.fecha
    ...
    def __repr__(self):
        return f'<Evento {self.id_evento} | {self.nombre} | Salon {self.id_salon} | {self.fecha}>'